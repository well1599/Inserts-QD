...
when UPPER(tables) like 'S_STBR_DATX' then 'ASSETS'
...
when UPPER(tables) like 'S_STBR_DTG%' then 'RELACOES INSTITUCIONAIS'
when lower(tabelas) like 'g_stbr_ofin%' then 'OPEN FINANCE'
when lower(tabelas) like 'h_stbr_dri_dri_pf%' then 'RISCOS'
when lower(tables) like 'h_stbr_dfs_dbnjfn%' then 'FINANÇAS'
when lower(tabelas) like 'h_stbr_dfs_db_nme%' then 'FINANÇAS'
when lower(tabelas) like 'gstbr_quod' then 'BUREAU'
WHEN lower(tables) like 'h_stbr_dfs_%' then 'FINANCES'
WHEN lower(tabela) like 'h_stbr_dri_cra%' then 'RISCOS'
WHEN lower(tabela) like 's_stbr_ofout%' then 'OPEN FINANCE'
WHEN lower(tabela) like 's_stbr_ofout%' then 'OPEN FINANCE'
end as DOMINIO
